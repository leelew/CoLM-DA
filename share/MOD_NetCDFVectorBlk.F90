...
MODULE MOD_NetCDFVector
   ...
   INTERFACE ncio_read_vector
      ...
      MODULE procedure ncio_read_vector_int32_2d 
   ...
   INTERFACE ncio_write_vector
      ...
      MODULE procedure ncio_write_vector_int32_2d 
...
CONTAINS
   ...
   !---------------------------------------------------------
   SUBROUTINE ncio_read_vector_int32_2d ( &
   ...
   logical :: any_data_exists
      ...
      IF (p_is_worker) THEN
         ...
         DO iblkgrp = 1, pixelset%nblkgrp
            ...
            CALL mpi_scatterv ( &
               MPI_INULL_P, MPI_INULL_P, MPI_INULL_P, MPI_INTEGER, & ! insignificant on workers
               rbuff, ndim1*pixelset%vecgs%vlen(iblk,jblk), MPI_INTEGER, &
               p_root, p_comm_group, p_err)
   ...
   END SUBROUTINE ncio_read_vector_int32_2d
   ...
   !---------------------------------------------------------
   SUBROUTINE ncio_write_vector_int32_2d ( &
   ...
   integer, allocatable :: sbuff(:,:), rbuff(:,:)
      ...
      IF (p_is_io) THEN
         ...
         DO iblkgrp = 1, pixelset%nblkgrp
            ...
            CALL mpi_gatherv (MPI_IN_PLACE, 0, MPI_INTEGER, &
               ...
               ndim1*pixelset%vecgs%vdsp(:,iblk,jblk), MPI_INTEGER, &
               p_root, p_comm_group, p_err)
#else
            istt = pixelset%vecgs%vstt(iblk,jblk)
            iend = pixelset%vecgs%vend(iblk,jblk)
      ...
      IF (p_is_worker) THEN
         ...
         DO iblkgrp = 1, pixelset%nblkgrp
            ...
            CALL mpi_gatherv ( &
               sbuff, ndim1*pixelset%vecgs%vlen(iblk,jblk), MPI_INTEGER, &
               MPI_INULL_P, MPI_INULL_P, MPI_INULL_P, MPI_INTEGER, & ! insignificant on workers
               p_root, p_comm_group, p_err)
   ...
   END SUBROUTINE ncio_write_vector_int32_2d
